...
class BertTrainer(Trainer):
    def __init__(self, args):
        ...
        print(f"Starting Time: {datetime.datetime.now():%F %T}")
    ...
    def report_and_reset_stats(self):
        ...
        #entrenamiento terminado tiempo
    def finish_training(self):
        end_ts = time.time()
        elapsed = end_ts - self.launch_ts
        h, rem = divmod(int(elapsed), 3600)
        m, s = divmod(rem, 60)
        print(f"[{datetime.datetime.now():%F %T}] Finished – duration {h:02d}:{m:02d}:{s:02d}")
