...
def mask_seq(src, tokenizer, whole_word_masking, span_masking, span_geo_prob, span_max_length):
    ...
    num_to_predict = max(1, int(round(len(src_no_pad) * 0.30)))
    tgt_mlm = []
    for index_set in tokens_index:
        ...
        if whole_word_masking:
            ...
            for j in range(mask_len):
                ...
                #         if rdi not in [vocab.get(CLS_TOKEN), vocab.get(SEP_TOKEN),vocab.get(PKTSEP_TOKEN), vocab.get(MASK_TOKEN), PAD_ID]:
                #             break
                #     src[i + j] = rdi
        elif span_masking:
            i = index_set[0]
...
class PopDataset(Dataset):
    """
    Cada instancia:
      src       : IDs (padded a seq_length)
      pop_pos   : [3] índices de los primeros PKTSEP en 'src' (-1 si no existe)
      pop_lbl   : [3]   1-3 = orden real, 0 = no válida
    """
    def worker(self, proc_id, start, end):
        print(f"Worker {proc_id} building POP dataset ...")
        set_seed(self.seed)
        writer = open(f"dataset-tmp-{proc_id}.pt", "wb")
        pkt_id  = self.vocab.get(PKTSEP_TOKEN)

        pos = 0
        with open(self.corpus_path, encoding="utf-8") as fh:
            while pos < start: fh.readline(); pos += 1
            while True:
                line = fh.readline(); pos += 1
                if not line: break
                tokens = self.tokenizer.convert_tokens_to_ids(
                             self.tokenizer.tokenize(line))
                # localiza separadores
                pkt_idx = [i for i,t in enumerate(tokens) if t == pkt_id]
                n = len(pkt_idx)
                if n < 2:               # 1 o 0 paquetes  descarta
                    if pos >= end: break
                    continue
                ...
                # construir secuencia reordenando los k paquetes
                def slice_pkt(j):
                    start = 0 if j==0 else first_k[j-1]+1
                    end   = first_k[j]+1
                    return tokens[start:end]

                new_seq = []
                for j in order: new_seq += slice_pkt(j)
                new_seq += tokens[first_k[k-1]+1:]

                # etiquetas
                pop_lbl = [order.index(i)+1 for i in range(k)] + [0]*(3-k)
                # pop_pos después de reordenar
                pkt_pos = [i for i,t in enumerate(new_seq) if t == pkt_id]
                pop_pos = pkt_pos[:k] + [-1]*(3-k)

                # padding
                new_seq = new_seq[:self.seq_length]
                while len(new_seq) < self.seq_length:
                    new_seq.append(PAD_ID)

                pickle.dump((new_seq, pop_pos, pop_lbl), writer)
                if pos >= end: break
        writer.close()

# dataloader para POP
class PopDataLoader(DataLoader):
    def __iter__(self):
        while True:
            while self._empty():
                self._fill_buf()

            batch = self.buffer[self.start:self.start + self.batch_size]
            self.start += self.batch_size
            if not batch:
                continue
            ...
            src, pos, lbl = [], [], []
            for s, p, l in batch:
                src.append(torch.tensor(s, dtype=torch.long))
                pos.append(torch.tensor(p, dtype=torch.long))
                lbl.append(torch.tensor(l, dtype=torch.long))

            src_t = torch.stack(src, dim=0)   # [B, L]
            pos_t = torch.stack(pos, dim=0)   # [B, 3]
            lbl_t = torch.stack(lbl, dim=0)   # [B, 3]

            yield src_t, pos_t, lbl_t
