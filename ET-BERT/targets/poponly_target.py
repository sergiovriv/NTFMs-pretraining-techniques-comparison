...
class PopOnlyTarget(nn.Module):
    """
    Cabeza exclusiva para Packet-Order Prediction.
    Recibe:
        seq_out : [B, L, H]
        pop_pos : [B, 3] índices de PKTSEP (−1 si inválido)
        pop_lbl : [B, 3] etiquetas 1-3, 0 = sin POP
    Devuelve:
        loss, correct, denom  (para imprimir accuracy)
    """
    ...
    def forward(self, seq_out, _, pop_pos, pop_lbl):
        B, L, H = seq_out.size()
        pop_pos = pop_pos.clamp(min=0, max=L-1)        # ← defensa
        denom = (pop_lbl > 0).sum()
        total_loss = 0.0
        total_correct = 0
