...
class MlmTrainer(Trainer):
    def __init__(self, args):
        ...
        print(f"[{datetime.datetime.now():%F %T}] MlmTrainer launched")
    ...
    def report_and_reset_stats(self):
        # tokens procesados hasta ahora
        done_tokens = self.batch_size * self.seq_length * self.report_steps
        if self.dist_train:
            done_tokens *= self.world_size

        # tiempo actual y transcurrido desde el inicio
        now = time.time()
        elapsed = now - self.launch_ts
        h, rem = divmod(int(elapsed), 3600)
        m, s = divmod(rem, 60)
        ts_str = datetime.datetime.now().strftime("%F %T")

        # impresion de estadisticas con timestamp
        print(f"{ts_str} | {h:02d}:{m:02d}:{s:02d} since start"
              f" | {self.current_step:8d}/{self.total_steps:8d} steps"
              f" | {done_tokens / (time.time() - self.start_time):8.2f} tokens/s"
              f" | loss {self.total_loss / self.report_steps:7.2f}"
              f" | acc: {self.total_correct / self.total_denominator:3.3f}")
...
class BertTrainer(Trainer):
    def __init__(self, args):
        ...
        self.launch_ts = time.time()
        print(f"[{datetime.datetime.now():%F %T}] BertTrainer launched")
    ...
    def report_and_reset_stats(self):
        ...
        #hora actual y tiempo desde inicio
        now = time.time()
        elapsed = now - self.launch_ts
        h, rem = divmod(int(elapsed), 3600)
        m, s = divmod(rem, 60)
...
#### PopTrainer ###############################################################
class PopTrainer(Trainer):
    """
    Trainer exclusivo para Packet-Order Prediction (POP).
    batch = (src, pop_pos, pop_lbl)
    """
    def __init__(self, args):
        super().__init__(args)
        self.total_correct = 0.0           # paquetes bien clasificados
        self.total_denominator = 0.0       # paquetes evaluables
        self.launch_ts = time.time()
        print(f"[{datetime.datetime.now():%F %T}] PopTrainer launched")

 
    def forward_propagation(self, batch, model):
        src, pop_pos, pop_lbl = batch          # src = [B,L] (IDs)
    
        if src.dim() == 1:                     # garantiza batch dim
            src      = src.unsqueeze(0)
            pop_pos  = pop_pos.unsqueeze(0)
            pop_lbl  = pop_lbl.unsqueeze(0)
    
        seg = torch.zeros_like(src)            # segmentos todo-cero
    
        # 1) Embeddings: [B,L,H]
        emb = model.embedding(src, seg)
    
        # 2) Encoder recibe EMBEDDINGS, devuelve [B,L,H]
        seq_out = model.encoder(emb, seg)      # NO indirection extra
        if isinstance(seq_out, tuple):         # por si devuelve (hidden, pooled,…)
            seq_out = seq_out[0]
    
        # 3) POP loss
        loss, correct, denom = model.target(
            seq_out, None, pop_pos.clamp(min=0, max=seq_out.size(1)-1), pop_lbl)
    
        # Acumula métricas
        self.total_loss        += loss.item()
        self.total_correct     += correct.item()
        self.total_denominator += denom.item()
    
        # Normaliza por accumulation_steps
    ...
    def report_and_reset_stats(self):
        ...
        acc = (self.total_correct / self.total_denominator) if self.total_denominator else 0.0
        print(f"{ts_str} | {h:02d}:{m:02d}:{s:02d} since start"
              f" | {self.current_step:8d}/{self.total_steps:8d} steps"
              f" | {done_tokens / (time.time() - self.start_time):8.2f} tokens/s"
              f" | loss {self.total_loss / self.report_steps:7.4f}"
              f" | acc: {acc:3.3f}")
