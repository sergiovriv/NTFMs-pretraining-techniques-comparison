...
def evaluate(args, dataset, print_confusion_matrix=False):
    ...
    if print_confusion_matrix:
        ...
        torch.set_printoptions(profile="full", precision=0)
        # nos aseguramos de que no se trunque la salida x pantalla
        print(confusion)
        torch.set_printoptions(profile="default")
        #volvemos al modo default
        
        
        cf_array = confusion.numpy()
        with open("/mnt/ntfms/ET-BERT/fine-tuning/results/conf-matrix",'w') as f:
...
def main():
    ...
    try:
        unk_id = args.tokenizer.vocab.get(UNK_TOKEN, None)   
    except Exception:
        unk_id = None
    
    if unk_id is not None:
        total_tokens = (src != 0).sum().item()            
        unk_tokens   = (src == unk_id).sum().item()
        ratio = 0.0 if total_tokens == 0 else 100.0 * unk_tokens / total_tokens
        print(f"[Sanity] UNK id: {unk_id} | UNK tokens: {unk_tokens}/{total_tokens} ({ratio:.2f}%)")
    else:
        print("[Sanity] No UNK id found in vocab (revisa tokenizer/vocab).")
    ...
    # shuffle before every epoch to destroy patterns
        perm = torch.randperm(instances_num)
        src = src[perm]
        tgt = tgt[perm]
        seg = seg[perm]
        if soft_tgt is not None:
            soft_tgt = soft_tgt[perm]
